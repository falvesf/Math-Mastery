...
async function run() {
    const { data, error } = await supabase.storage.from('uploads').list('ranks');
    if (error) {
        console.log("Error listing storage:", error.message);
    } else {
        console.log(`Found ${data.length} files in ranks/`);
    }
}
run();
