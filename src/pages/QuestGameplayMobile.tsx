...
export default function QuestGameplay() {
  ...
  // Sai da missão com a música parando gradualmente + transição de batalha antes de navegar
  const exitToCamp = () => {
    fadeOutMusic(900);
  ...
  const playPlayerAttackSound = (isCrit = false) => {
    const weapon = playerEquippedItems.find(i =>
      ...
      ((i as any).criticalSoundUrl || (i as any).battleSoundUrl)
    );
...
const dealTransformDamageToPlayer = (damage: number) => {
  ...
  // Carrega os 4 modos do Supabase ao montar, sem que nenhum sobrescreva os demais ou o trabalho ativo do usuário
  useEffect(() => {
    let isMounted = true;
  ...
  const handleSaveArenaDebug = async () => {
    ...
    // Salva na chave específica dos 4 modos
    const { data: existing } = await supabase.from('system_collections').select('id').eq('collection_name', 'arena_debug').eq('doc_id', docId).limit(1);
    let error;
    if (existing && existing.length > 0) {
      const res = await supabase.from('system_collections').update({ data: dataToSave }).eq('collection_name', 'arena_debug').eq('doc_id', docId);
      error = res.error;
    } else {
...
*/
  ...
  const finishGame = async (isWin: boolean, finalXp: number, customMessage?: string) => {
    ...
    if (finalRewards.coins > 0 || finalRewards.items.length > 0) {
      ...
      if (isWin && (actualXpGained > 0 || earnedCoins > 0)) {
        // Recompensa VALIDADA NO SERVIDOR (anti-fraude via console): apply_quest_reward
        // confere tetos e anti-spam antes de aplicar.
        ...
        if (rewardErr) {
          // Fallback seguro: se a migration da função não rodou, usa o comportamento antigo.
  ...
  const collectCoin = (coin: { id: number; x: number; y: number; value: number }) => {
    if (!userData) return;
    // Recompensa VALIDADA NO SERVIDOR (teto + anti-spam)
  ...
  // FOGO/SANGRAMENTO: moedas caídas queimam/somem após ~4s.
  useEffect(() => {
  ...
  // 1% de chance do TRONCO cair PRIMEIRO → vantagem de eliminar 1 alternativa por questão;
  // nesse caso os braços caem junto com o tronco.
  useEffect(() => {
    ...
    <div className="app-container" style={{ 
      ...
      <div style={{ position: 'relative', zIndex: 10, width: '100%', maxWidth: '100%', margin: '0 auto', display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden', flexShrink: 0 }}>
        ...
        <div style={{ padding: '0.35rem 0.75rem', display: 'flex', flexDirection: 'column', gap: '0.25rem', background: 'rgba(0, 0, 0, 0.5)', backdropFilter: 'blur(10px)', borderBottom: '1px solid var(--border-glass)', flexShrink: 0 }}>
          ...
          {/* Row 2: Stats + debug (itens utilizáveis foram movidos para a barra fixa inferior) */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', flexWrap: 'nowrap', overflowX: 'auto' }}>
            ...
            {gameState === 'playing' && (
              <>
                ...
                >
                  <span>{arenaRenderMode === '3d' ? '🧱 3D' : '🖼️ 2D'}</span>
                </button>
              ...
              {monsterAnim === 'death-slice' ? (
                ...
                <div className={`quest-arena-monster-inner ${monsterHealPulse ? 'monster-healing-active' : ''}`} style={{ position: 'relative', display: 'inline-block' }}>
                  ...
                  {(() => {
                      ...
                      return (
                        <div style={{ transformOrigin: 'bottom center', marginBottom: `-${trFeetBottom}px` }}>
                          <div className={animCls || undefined} style={{ position: 'relative' }}>
                            ...
                            {/* Badge da transformação — perto do topo do modelo */}
                    ...
                    const modelCfg = getMonsterModelCfg(modelUrl);
          ...
          {gameState === 'result' && (
            <div className="glass-panel quest-victory-panel" style={{ margin: 'auto', width: '100%', maxWidth: '600px', textAlign: 'center', animation: 'epicZoom 0.5s ease-out', border: won ? '2px solid var(--gold-primary)' : '2px solid var(--accent-red)' }}>
              ...
              <div style={{ display: 'flex', gap: '0.8rem', justifyContent: 'center', flexWrap: 'wrap' }}>
                <button className="login-btn" onClick={() => exitToCamp()} disabled={saving} style={{ background: 'var(--btn-bg)', color: 'var(--text-primary)', border: '1px solid var(--border-glass)', padding: '0.8rem 2rem', fontSize: '1.1rem' }}>
...
chestRotY={selectedChestModel?.chestRotY}
                ...
                <div style={{ textAlign: 'center', animation: 'epicZoom 0.5s ease-out' }}>
                  ...
                  <button className="login-btn" onClick={() => exitToCamp()} style={{ marginTop: '4rem', background: 'var(--gold-primary)', color: 'var(--text-on-gold, #000000)', padding: '1rem 3rem', fontSize: '1.2rem', animation: 'popInChest 0.3s ease-out forwards', animationDelay: '1s', opacity: 0 }}>
        ...
        {gameState === 'playing' && powerups.length > 0 && (() => {
          ...
          return (
            <div style={{ flexShrink: 0, width: '100%', background: 'rgba(0,0,0,0.55)', backdropFilter: 'blur(10px)', borderTop: '1px solid var(--border-glass)', padding: '0.4rem 0.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', zIndex: 25 }}>
              <button
                type="button"
                onClick={() => setItemScrollIndex(Math.max(0, scroll - VISIBLE))}
                disabled={!canLeft}
                aria-label="Itens anteriores"
                style={{ width: '28px', height: '28px', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: canLeft ? 'rgba(245,158,11,0.2)' : 'transparent', border: canLeft ? '1px solid var(--gold-primary)' : '1px solid transparent', borderRadius: '50%', color: canLeft ? 'var(--gold-primary)' : 'transparent', cursor: canLeft ? 'pointer' : 'default', padding: 0 }}
              >
                <ChevronLeft size={18} />
              </button>
                ...
                {visible.map(p => {
                  ...
                  return (
                    <button
                      ...
                      style={{ position: 'relative', width: '44px', height: '44px', background: 'rgba(0,0,0,0.5)', border: '1px solid var(--border-glass)', borderRadius: '8px', cursor: 'pointer', padding: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}
                    >
                      {p.itemImageUrl ? (
                        <img src={getSafeUrl(p.itemImageUrl)} alt={p.itemTitle} style={{ width: '32px', height: '32px', borderRadius: '4px', objectFit: 'contain' }} />
                      ) : (
              ...
              <button
                ...
                style={{ width: '28px', height: '28px', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: canRight ? 'rgba(245,158,11,0.2)' : 'transparent', border: canRight ? '1px solid var(--gold-primary)' : '1px solid transparent', borderRadius: '50%', color: canRight ? 'var(--gold-primary)' : 'transparent', cursor: canRight ? 'pointer' : 'default', padding: 0 }}
              >
