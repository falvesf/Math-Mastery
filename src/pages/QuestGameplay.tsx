...
export default function QuestGameplay() {
  ...
  const handleMonsterCanvasReady = useCallback((c: HTMLCanvasElement | null) => {
    monsterCanvasRef.current = c;
  }, []);
...
const dealTransformDamageToPlayer = (damage: number) => {
  ...
  // Quando as alternativas caem na mesma altura dos bonecos (ficam por cima e não dá
  // para clicar), movemos as respostas para BAIXO da arena. Se a pergunta ficar acima
  // dos bonecos, mantemos no topo.
  ...
  const handleArenaDebugChange = (newConfig: ArenaDebugConfig) => {
    hasUserModifiedDebugRef.current = true;
    setConfigsByMode(prev => {
      ...
      try {
        ...
        // Marca como "não salvo": impede que o carregamento do servidor sobrescreva
        // este modo (evitando perder ajustes ao trocar de modo/remontar a tela).
  ...
  // cancela o override manual para o painel voltar a seguir a tela.
  ...
  useEffect(() => {
    ...
    const loadAllConfigs = async () => {
      try {
        ...
        const data2dDesk = parseDoc(res2dDesk, null);
        const data2dMob = parseDoc(res2dMob, null);
        ...
        setConfigsByMode(prev => {
          ...
          if (data3dMob && shouldUpdate('3d_mobile')) {
          ...
          if (data2dDesk && shouldUpdate('2d_desktop')) {
          ...
          if (data2dMob && shouldUpdate('2d_mobile')) {
  ...
  useEffect(() => {
    // Não re-executar se já estamos jogando ou em resultado ou se a quest já foi carregada
    ...
    const fetchQuest = async () => {
      try {
        ...
        const qData = {
          ...
          battleBgUrl: snap.battleBgUrl || snap.battle_bg_url || null,
          arena3D: (snap.arena3D ?? snap.arena_3d ?? (typeof (snap.battleBgUrl || snap.battle_bg_url) === 'string' && (snap.battleBgUrl || snap.battle_bg_url || '').startsWith('voxel:'))) || false,
          battleBiome: snap.battleBiome || snap.battle_biome || (((snap.battleBgUrl || snap.battle_bg_url || '').startsWith('voxel:')) ? (snap.battleBgUrl || snap.battle_bg_url).replace('voxel:', '') : 'plains'),
...
*/
  ...
  const handleAnswer = async (optIndex: number) => {
    ...
    if (isCorrect) {
      ...
      const tr = transformRef.current;
      const damage = computeMonsterDamageToPlayer(currentHearts, isHardcore, tr, isMonsterCrit);
      ...
      // Executa o acerto físico no jogador (aplica dano, escudo, penalidades e morte)
      ...
      if (decision.type === 'ranged') {
        // Monstro NÃO teleporta para frente do jogador. Ergue os braços e lança o projétil em arco!
        setMonsterBodyThrow(true);
        setMonsterProjectile({
          id: Date.now(),
      ...
      if (decision.type === 'special') {
        ...
        if (decision.proceduralType) {
          ...
          if (decision.proceduralType === 'jump_slam') {
            // Pulo alto com estrondo sísmico no impacto (~750ms)
            setTimeout(() => {
              setArenaQuake(true);
              setShockwaveActive(true);
              dropCoinsIfDamaged();
        ...
        // Animação nativa do GLB (ex: Wither shoot, Aranha attack)
  ...
  const checkAndDropMonsterItem = () => {
    ...
    for (const drop of shuffled) {
      ...
      if (roll <= drop.dropChance) {
        ...
        playCoinCollect((activeCoinModel as any)?.coinSoundUrl);
        // Regra "apenas 1 por vez": interrompe assim que 1 item é sorteado
        break;
      }
    }
  };
  ...
  const collectBattleItem = async (drop: DroppedBattleItem) => {
    ...
    if (userData?.uid && !isStudyMode) {
      try {
        ...
        console.error("Erro ao registrar drop de monstro:", err);
      }
    }
  };
...
const applyMonsterEffectToPlayer = (effect: string, damageInflicted: number = 1) => {
  ...
  switch (effect) {
    ...
    case 'burn':
      ...
      break;
    case 'electric':
      setPlayerElectricTurns(3);
      break;
    case 'freeze':
      setPlayerFrozenAt(Date.now() + 1800);
      break;
    case 'impact':
      setArenaQuake(true);
...
const rollMonsterAttack = () => {
  ...
  // Cura: se o monstro estiver "morrendo" (últimas questões) e configurado → chance de curar
    ...
    <div className="app-container" style={{ 
      ...
      <div style={{ position: 'relative', zIndex: 10, width: '100%', maxWidth: '1000px', margin: '0 auto', display: 'flex', flexDirection: 'column', height: '100%' }}>
            ...
            {gameState === 'playing' && (
              <>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.2rem', background: 'rgba(0,0,0,0.5)', padding: '0.5rem 1rem', borderRadius: '20px', border: '1px solid #ef4444' }}>
                  {Array.from({ length: Math.ceil(currentHearts) }).map((_, i) => {
                    ...
                    return (
                      <span key={i} style={{ position: 'relative', display: 'inline-block', lineHeight: 0 }}>
                        <Heart size={18} fill="rgba(239, 68, 68, 0.12)" color="#ef4444" />
                        {!isHalf && (
                  ...
                  {healAuraTurns > 0 && (
                    <span title={`Aura de cura ativa por ${healAuraTurns} turno(s)`} style={{ display: 'flex', alignItems: 'center', gap: '0.2rem', color: '#34d399', fontWeight: 'bold', fontSize: '0.75rem', marginLeft: '0.4rem' }}>
                      💚 +{healAuraTurns}
                    </span>
                  )}
                </div>

                <div title="Moedas que você tem" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'rgba(0,0,0,0.5)', padding: '0.5rem 1rem', borderRadius: '20px', border: '1px solid var(--gold-primary)' }}>
                  {activeCoinModel && activeCoinModel.url ? (
                    <img src={activeCoinModel.open_url || activeCoinModel.url} alt="Moeda" style={{ width: '22px', height: '22px', objectFit: 'contain' }} />
                  ) : (
                    <Coins size={18} color="var(--gold-primary)" fill="rgba(245, 158, 11, 0.4)" />
                  )}
        ...
        {(() => {
          ...
          return gameState === 'playing' && (
            <div ref={arenaRef} className={`battle-arena-bg quest-arena ${arenaRenderMode === '3d' ? 'is-3d-arena' : ''} ${arenaQuake ? 'arena-quake' : ''}`} style={{ '--attack-dist': arenaRenderMode === '3d' ? (effectiveAttackDist > 0 ? `${effectiveAttackDist}px` : 'var(--shadow-attack-dist, 200px)') : `${finalAttackDist2D}px`, '--arena-char-bottom-padding': '60px', '--player-lift-x': `${arenaDebug.playerOffsetX3D || 0}px`, '--monster-lift-x': `${arenaDebug.monsterOffsetX3D || 0}px`, '--player-lift-3d': `${arenaDebug.playerOffsetY3D || 0}px`, '--monster-lift-3d': `${arenaDebug.monsterOffsetY3D || 0}px`, position: 'relative', width: '100%', display: 'block', paddingTop: '130px', paddingBottom: '60px', borderBottom: '1px solid var(--border-glass)', flexShrink: 0, zIndex: 20, userSelect: 'none', WebkitUserSelect: 'none' } as any}>
            {/* Cenário: 3D Voxel Minecraft ou Imagem 2D Clássica */}
            {arenaRenderMode === '3d' ? (
              <VoxelArena3D
                deviceMode={effectiveDevice}
                // Bioma: sempre o da missão; fallback = Planícies. Admin com o painel Debug
            ...
            {/* Monster Loot Drops na arena com Auras de Raridade */}
            ...
            <div 
              ...
              style={{
                ...
                bottom: arenaRenderMode === '3d'
                  ...
                  : `calc(60px + ${(arenaDebug.playerOffsetY ?? 0)}px)`,
                width: '160px',
                marginLeft: '-80px',
                display: 'flex',
                ...
                alignItems: 'center',
                gap: '0.5rem',
                transform: playerAnim === 'hurt' ? 'translateX(-20px) rotate(-10deg)' : undefined,
                transition: playerAnim.startsWith('attack') ? 'none' : 'transform 1s cubic-bezier(0.175, 0.885, 0.32, 1.275)',
                zIndex: (playerAnim.startsWith('attack') || playerAnim === 'idle-victory' || playerAnim.startsWith('victory-')) ? 30 : (monsterAnim.startsWith('death-') ? 20 : 26),
                pointerEvents: 'none'
              }}
            >
              {playerBubble && (
            ...
            {/* Battle Message */}
            <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', zIndex: 20, opacity: battleMessage ? 1 : 0, transition: 'opacity 0.3s', pointerEvents: 'none' }}>
            ...
            <div 
              ...
              style={{
                ...
                bottom: arenaRenderMode === '3d'
                  ...
                  : `calc(60px + ${(arenaDebug.monsterOffsetY ?? 0)}px)`,
                width: '160px',
                marginLeft: '-80px',
                display: 'flex',
                flexDirection: 'column',
                alignItems: 'center',
                gap: '0.5rem',
                transform: monsterAnim === 'hurt' ? 'translateX(20px) rotate(10deg)' : undefined,
                transition: monsterAnim.startsWith('attack') ? 'none' : 'transform 1s cubic-bezier(0.175, 0.885, 0.32, 1.275)',
                zIndex: (monsterAnim === 'attack-fatal' || monsterAnim === 'attack-fatal-slow') ? 35 : (monsterAnim.startsWith('attack') || monsterAnim.startsWith('death-') || monsterProceduralAnim || monsterBodyThrow) ? 30 : 26,
                pointerEvents: 'none'
              }}
            >
              {monsterBubble && (
              ...
              {monsterAnim === 'death-slice' ? (
                <div className="quest-arena-avatars" style={{ position: 'relative', width: '160px', height: effectiveMonsterModelUrl ? '240px' : '228px' }}>
                  {/* Nome do monstro - acompanha death-slice */}
                  <div style={{ position: 'absolute', top: `${arenaDebug.monsterNameY - (effectiveMonsterZoom - 1) * (effectiveMonsterModelUrl ? 140 : 216)}px`, left: '50%', transform: 'translateX(-50%)', zIndex: 5, whiteSpace: 'nowrap', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px' }}>
                ...
                >
                  ...
                  <div style={{ position: 'absolute', top: `${arenaDebug.monsterNameY - (effectiveMonsterZoom - 1) * ((quest?.monsterModelUrl || quest?.monsterAvatarConfig?.customModelUrl) ? 140 : 216)}px`, left: '50%', transform: 'translateX(-50%)', zIndex: 5, whiteSpace: 'nowrap', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px', opacity: monsterAnim.startsWith('death-') ? 0.3 : 1, transition: 'opacity 2s' }}>
                ...
                <div className={`quest-arena-monster-inner ${monsterHealPulse ? 'monster-healing-active' : ''}`} style={{ position: 'relative', display: 'inline-block' }}>
                  ...
                  {(() => {
                    ...
                    // MONSTRO TRANSFORMADO (GLB do animal)
                    if (tr) {
                      ...
                      return (
                        <div style={{ transformOrigin: 'bottom center', marginBottom: `-${trFeetBottom}px` }}>
                          <div className={animCls || undefined} style={{ position: 'relative' }}>
                            ...
                            <CustomModelViewer modelUrl={animalUrl} size={viewerSize} configRotY={rotY} animation="none" role="monster" effectTint={tint} preserveDrawingBuffer onCanvasReady={handleMonsterCanvasReady} />
      ...
      {gameState === 'playing' && showDebugPanel && canArenaDebug('arena_debug', 'view') && (
        <ArenaDebugPanel
          ...
          currentMonsterModelUrl={effectiveMonsterModelUrl}
        />
      )}
