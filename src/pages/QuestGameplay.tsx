...
*/
  ...
  const handleAnswer = async (optIndex: number) => {
    ...
    if (isCorrect) {
      ...
      const isFatalForPlayer = !hasShield && isMonsterDamageFatal(damage, currentHearts);
...
const applyMonsterEffectToPlayer = (effect: string) => {
  ...
  switch (effect) {
    ...
    case 'electric':
      ...
      break;
    case 'freeze':
      setPlayerFrozenAt(Date.now() + 1600);
      break;
    case 'heal':
  ...
  return (
    <div className="app-container" style={{ 
      ...
      <div style={{ position: 'relative', zIndex: 10, width: '100%', maxWidth: '1000px', margin: '0 auto', display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        {/* Battle Arena Fixed */}
        {gameState === 'playing' && (
          <div ref={arenaRef} className="battle-arena-bg quest-arena" style={{ '--attack-dist': `${Math.max(50, arenaWidth - 340)}px`, position: 'relative', width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', paddingTop: '130px', paddingBottom: '60px', borderBottom: '1px solid var(--border-glass)', flexShrink: 0, zIndex: 20, userSelect: 'none', WebkitUserSelect: 'none' } as any}>
