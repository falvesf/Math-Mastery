          ...
          {selectedEquippedItem && (
            <div style={{ border: '1px solid rgba(245,158,11,0.4)', borderRadius: '8px', padding: '0.7rem' }}>
              ...
              {[
                ...
                <div key={key} style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', marginBottom: '0.2rem' }}>
                  <span style={{ width: '46px', color: '#f59e0b', fontFamily: 'monospace', fontWeight: 'bold', fontSize: '0.72rem', flexShrink: 0 }}>{label}</span>
                  <input type="range" min={min} max={max} step={step} value={itemTransform[key] ?? 0} onChange={(e) => setItemTransform(prev => ({ ...prev, [key]: parseFloat(e.target.value) }))} style={{ flex: 1, minWidth: '60px', accentColor: '#f59e0b' }} />
                  <input
                    type="number"
                    step={step}
                    value={Number((itemTransform[key] ?? 0).toFixed(key.startsWith('rot') ? 3 : 2))}
                    onChange={(e) => {
                      const v = parseFloat(e.target.value);
                      if (!isNaN(v)) setItemTransform(prev => ({ ...prev, [key]: v }));
                    }}
                    style={{
                      width: '56px',
                      textAlign: 'right',
                    ...
                    title="Digite um valor exato ou use as setas do teclado"
                  />
                </div>
              ))}
